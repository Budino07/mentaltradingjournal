...
} from '@/utils/psychology/coreTraitsAnalysis';
...
export const CoreTraitsMatrix = ({ emotionalData: providedData }: CoreTraitsMatrixProps) => {
  ...
  const colors = {
    ...
    connection: '#0284c7',
    growth: '#ca8a04',
    conviction: '#f59e0b',  // Amber for conviction
  ...
  const traitDescriptions = {
    ...
    conviction: 'Strong belief in setups and decisions, leading to holding through volatility and executing without hesitation.',
    focus: 'Staying present, mentally engaged, and undistracted during the trading process.',
    confidence: 'Belief in oneself, executing with clarity and maintaining emotional strength regardless of outcomes.',
  ...
  const traitRecommendations = {
    ...
    conviction: 'Balance conviction with adaptability. Strong beliefs should be evidence-based and adjusted with new information.',
    focus: 'Create a distraction-free trading environment. Consider techniques like timeboxing or the Pomodoro method for sustained focus.',
    confidence: 'Build confidence through consistent process execution, not just outcomes. Keep a record of successful decisions.',
  ...
  const getCoreTraitIcon = (trait: string) => {
    switch (trait) {
      ...
      case 'growth': return '🌱';
      case 'conviction': return '💪';  // Strong arm for conviction
      case 'focus': return '🎯';       // Target for focus
      case 'confidence': return '🌟';  // Star for confidence
      default: return '❓';
    }
  };
