      ...
      <div className="flex flex-col gap-2">
        ...
        {payload[0]?.payload?.dailyPnL !== undefined && (
          <div className="flex items-center gap-2 text-sm mt-1 pt-1.5 border-t border-white/10">
            <div className={`w-2.5 h-2.5 rounded-full ${payload[0].payload.dailyPnL >= 0 ? 'bg-green-500' : 'bg-red-500'}`} />
            <span className="text-white/75 dark:text-muted-foreground">Daily P&L:</span>
            <span className={`font-medium ${payload[0].payload.dailyPnL >= 0 ? 'text-green-500' : 'text-red-500'}`}>
              {valueFormatter 
                ? valueFormatter(payload[0].payload.dailyPnL)
                : `$${payload[0].payload.dailyPnL.toFixed(2)}`}
            </span>
          </div>
        )}
      </div>
